...
The performance test is performed on a similar setup as Stability test. The JMeter VM will be sending large number of REST requests to the PAP component and collecting the statistics.

Test Plan
---------

Performance test plan is the same as stability test plan above except for the few differences listed below .

- Increase the number of threads up to 5(simulating 5 users' behaviour at the same time)
...
Running/Triggering performance test will be the same as stability test. That is, launch JMeter pointing to corresponding *.jmx* test plan. The *API_HOST* , *API_PORT* , *PAP_HOST* , *PAP_PORT* are already set up in *.jmx*.
