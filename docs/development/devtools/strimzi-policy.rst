...
Enable communication over Kafka
-------------------------------
To build a custom Kafka Cluster, Set UseStrimziKafka in policy/value.yaml to false, Or donot have any Strimzi-Kafka policy configuration in oom/kubernetes/policy/

Following commands will create a simple custom kafka cluster, This strimzi cluster is not ONAP based Strimzi Kafka Cluster. Custom kafka cluster is established with ready to use commands from https://strimzi.io/quickstarts/
...
In case of a custom kafka cluster, topics have to be either manually created with below command Or programatically created with "allow.auto.create.topics = true" in Consumer config properties. Replace the topic below in the code block and create as many topics as needed for the component.
...
Following commands will ensure a quick check to see the Kafka producer and Kafka Consumer are working, with given Bootstrap server and topic.
