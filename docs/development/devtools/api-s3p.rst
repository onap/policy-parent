...
service by ingesting a steady flow of transactions of policy design API calls in a multi-thread fashion to
...
Specific performance tests were omitted in Guilin.   The JMeter script in the previous experiment injected
back to back traffic with 5 parallel threads with no pauses between them.   Since the JMeter threads operate
in synchronous mode (wait for a request's response before initiating the next one), the indirect backpressure the the API
component places on JMeter by means of the increasing response times, give us a good indication
of the performance of the API component.  Therefore for the described traffic mix, not much traffic should be added against the API
component without increasing average times of over 7 seconds.
