...
Other ONAP components exercised during the stability tests are:
...
The policy kubernetes charts were modified previous to the installation
to add the following script that enables the feature-controlloop-utils feature:
...
The tests were focused on the following use cases:
...
JMeter was run within a docker with the following command:
...
where approximately 1% of transactions are not completed as expected for
some use cases.   Average Execution Times are extended as well.
The unexpected results of the transactions seem to point to the
...
.. code-block:: bash
    ...
    # Common pattern in the audit.log of unexpected transaction completions
    ...
    # The "Target Lock was lost" is a common them in the unexpected results.
