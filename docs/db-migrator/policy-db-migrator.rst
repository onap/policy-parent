...
The script that runs ``db-migrator`` is part of the database setup and is in the following directory:

.. code::

   oom/kubernetes/policy/resources/config/db_migrator_policy_init.sh

It is configured to run an upgrade by default.
...
The first line can be changed to call either ``prepare_upgrade.sh`` or ``prepare_downgrade.sh`` depending on the user requirements.
