...
with remote networked remote entities typically using messaging (dmaap or ueb),
or http.
...
PDP-D supports the following networked infrastructures.   This is also referred as
...
source and sinks, to denote their directionality into or out of the *controller*.
...
configuration.  This configuration lines also contain specific
...
that provides means to share data across applications.   The
...
for observe and alter the default *PolicyEngine* behavior.
...
Therefore affecting *controllers*, and *endpoints*.   These capabilities are intended to be used for extensions,
...
and the <https://git.onap.org/policy/drools-applications>`__ repository for miscellaneous features
implementations.
...
*telemetry* API.   The *telemetry* shell wraps this APIs into for shell-like access using
...
Running PDP-D in the IDE
========================

PDP-D can be tested through the IDE.    The reader is referred to the following page for additional
information:
...
- ***.xml** to add override other .xml configuration that may be used for example by an *application*.
- ***.json** similarly *json* configuration that may be used by an *application*.
...
To put the controller directly in active mode at initialization, placed an *active.post.sh* script under the
...
Note that we are opening the *9696* telemetry API port to the outside world, mount the host directory where the *noop.pre.sh*
is located, and sourced in the environment variables into the PDP-D container.
