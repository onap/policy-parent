...
.. code-block:: bash
    ...
    controlLoop:
      version: 2.0.0
      controlLoopName: {{Unique ID for the Control Loop, must match one of the IDs defines in the list of policies below}}
...
Assuming the YAML is saved into a file by name "policy.yaml", use the below script to encode it:
...
The encoded YAML data from the above step needs to be substituted to the following payload template to create the operational policy.
