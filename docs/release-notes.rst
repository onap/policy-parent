...
* Decoupling ACMR/Clamp from Policy Framework
  ...
  (policy/common, policy/models). Previously, clamp inherited dependencies, utilities, REST clients, and exception classes from other PF repos. This created tight coupling that slowed down both clamp and PF development. Clamp now manages its own dependencies directly, making it independently buildable and releasable without requiring larger PF changes.
