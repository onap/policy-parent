...
Otherwise, it will complain that guard policy type cannot be found (does not exist). This naming restriction will disappear after 
we evolve to use well-formed TOSCA Service Template for guard policies and legacy policy design API is then deprecated.

.. swaggerv2doc:: swagger/operational-policy-api.json

Regarding DELETE APIs, we only expose API to delete one particular version of policy or policy type at a time for safety purpose. 
If client has the need to delete multiple or a group of policies or policy types, they have to delete one by one.  
